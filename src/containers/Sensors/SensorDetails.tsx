      ...
      <div className="py-6 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto">
          <div className="bg-yellow-50 p-4 rounded-md">
            <h3 className="text-sm font-medium text-yellow-800">{t('sensor_not_found')}</h3>
          </div>
          <div className="mt-4">
            <Button
              variant="outline"
    ...
    <div className="py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-semibold text-gray-900">{sensor.name}</h1>
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
          {/* Sensor Details */}
          <div className="p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4">{t('sensor_details')}</h2>
            <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
              ...
              <div>
                <dt className="text-sm font-medium text-gray-500">{t('area')}</dt>
                <dd className="mt-1 text-sm text-gray-900">
                  {area ? (
                    <Link 
                      to={`/areas/${sensor.areaId}`}
              ...
              {sensor.description && (
                <div className="sm:col-span-2">
                  ...
                  <dd className="mt-1 text-sm text-gray-900">{sensor.description}</dd>
                </div>
              )}

              {Object.keys(sensor.metadata || {}).length > 0 && (
                <div className="sm:col-span-2">
                  <dt className="text-sm font-medium text-gray-500 mb-2">{t('metadata')}</dt>
                  <dd className="mt-1">
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 className="text-lg font-medium text-gray-900">{t('delete_sensor')}</h3>
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        {t('delete_sensor_confirm', { name: sensor.name })}
                      </p>
                    </div>
                  </div>
                </div>
